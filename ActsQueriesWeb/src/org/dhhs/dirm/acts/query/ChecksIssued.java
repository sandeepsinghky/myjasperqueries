...
public class ChecksIssued implements QueryInterface
{
	...
	public String getQueryString()
	{
		...
		DBConnectManager.getQueryRegion() + ".FKKT_CHECK_COMP " + " WHERE DT_CHK_PRT BETWEEN '" + this.fromDate + "'" + " AND '" + this.toDate + "'" + " GROUP BY CD_DISB_TYPE";
	...
	public void sendEmail() throws QueryException
	{
		try
		{
			...
			em.setSubject("Acts - Automated Query Request (Checks Issued) Requested by:" + this.owner);
			if (this.pdfRequired)
			{
	...
	public void invoke(DataSource ds, QueryDateBean qdb) throws QueryException
	{
		...
		try
		{
			...
			try
			{
				String rf = this.path + System.getProperty("file.separator") + "ChecksIssued.jasper";
	...
	{
		this.path = (QueryPropertyLoader.getPath() + System.getProperty("file.separator") + "ChecksIssued");
