...
public class QueryTwo implements QueryInterface
{
	...
	private String			bcc;
	private String			cdCheckStat;
	private String			cdRsnAdj;
	private String			cdTypeJnlevt;
	...
	private String			className	= QueryTwo.class.getName();
	private String			path		= "";
	...
	{
		String sql = "SELECT DISTINCT  C.NB_CHK as Check,  B.DT_TRN as Date,  B.AM_TRN_RMNG as Amount,  A.NB_BTCH_JNLEVT as Batch,  A.NB_ITM_JNLEVT as Item   FROM " +

		DBConnectManager.getQueryRegion() + ".FKKT_JOURNAL_EVENT A, " + DBConnectManager.getQueryRegion() + ".FKKT_JOURNAL_TRAN B, " + DBConnectManager.getQueryRegion() + ".FKKT_CHECK_COMP C " + " WHERE A.CD_RSN_ADJ IN (" + this.cdRsnAdj + ")" + "   AND A.CD_TYPE_JNLEVT = '" + this.cdTypeJnlevt + "'" + "   AND A.NB_MO_JNLEVT = B.NB_MO_JNLEVT " + "   AND A.NB_BTCH_JNLEVT = B.NB_BTCH_JNLEVT " + "   AND A.NB_ITM_JNLEVT = B.NB_ITM_JNLEVT " + "   AND B.DT_TRN BETWEEN '" + this.fromDate + "'" + " AND '" + this.toDate + "'" + "   AND B.AM_TRN_RMNG > 0 " + "   AND B.NB_MO_JNLEVT_SRCE = C.NB_MO_JNLEVT " + "   AND B.NB_BTCH_JNLEVT_SRC = C.NB_BTCH_JNLEVT " + "   AND B.NB_ITM_JNLEVT_SRCE = C.NB_ITM_JNLEVT " + "   AND C.CD_CHK_STAT = '" + this.cdCheckStat + "' " + " ORDER BY Check";
	...
	public void sendEmail() throws QueryException
	{
		try
		{
			...
			em.setSubject("Acts - Automated Query Request (Checks cancelled & not reissued) Requested by:" + this.owner);
			if (this.pdfRequired)
			{
	...
	public void loadProperties() throws QueryException
	{
		try
		{
			...
			this.cdTypeJnlevt = configBundle.getString("QueryClass.cdTypeJnlevt");
			this.cdRsnAdj = configBundle.getString("QueryClass.cdRsnAdj");
			this.cdCheckStat = configBundle.getString("QueryClass.cdCheckStat");
	...
	public void invoke(DataSource ds, QueryDateBean qdb) throws QueryException
	{
		...
		buf.append("This message has been sent in response to your query request for Checks Cancelled and not reissued.If you did not request this service or believe this message has been sent to you in error, please contact ACTS Help Desk..\n\n");
		...
		try
		{
			...
			try
			{
				String rf = this.path + System.getProperty("file.separator") + "QueryTwoDef.jasper";
	...
	{
		this.path = (QueryPropertyLoader.getPath() + System.getProperty("file.separator") + "QueryTwo");
