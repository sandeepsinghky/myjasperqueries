...
public class QueryThree implements QueryInterface
{
	...
	{
		String sql = "SELECT   C.ID_PART as Mpi,  C.NM_PART_L as NameLast,  C.NM_PART_F as NameFirst,  A.DT_CHK_PRT as Date,  A.NB_CHK as Check,  A.AM_DISB as Amount,  B.AM_SUBACCT_BAL as Balance   FROM " +

		DBConnectManager.getQueryRegion() + ".FKKT_PARTICIPANT C, " + DBConnectManager.getQueryRegion() + ".FKKT_CHECK_COMP A, " + DBConnectManager.getQueryRegion() + ".FKKT_SUBACCOUNT B " + " WHERE A.CD_DISB_TYPE = '" + this.cdDisbType + "'" + "   AND A.ID_PAYEE = C.ID_PART " + "   AND B.NB_ACCT = C.ID_PART " + "   AND B.ID_SUBACCT = '" + this.idSubacct + "'" + "   AND B.AM_SUBACCT_BAL > 0 " + " UNION " + "SELECT  " + " C.ID_PART as Mpi, " + " C.NM_PART_L as NameLast, " + " C.NM_PART_F as NameFirst, " + " A.DT_CHK_PRT as Date, " + " A.NB_CHK as Check, " + " A.AM_DISB as Amount, " + " B.AM_SUBACCT_BAL as Balance " + "  FROM " + DBConnectManager.getQueryRegion() + ".FKKT_PARTICIPANT C, " + DBConnectManager.getQueryRegion() + ".FKKT_ARC_CHK_COMP A, " + DBConnectManager.getQueryRegion() + ".FKKT_SUBACCOUNT B " + " WHERE A.CD_DISB_TYPE = '" + this.cdDisbType + "'" + "   AND A.ID_PAYEE = C.ID_PART " + "   AND B.NB_ACCT = C.ID_PART " + "   AND B.ID_SUBACCT = '" + this.idSubacct + "'" + "   AND B.AM_SUBACCT_BAL > 0 " + " ORDER BY NameLast, NameFirst, Mpi, Date";
	...
	public void sendEmail() throws QueryException
	{
		try
		{
			...
			em.setSubject("Acts - Automated Query Request (Unpaid Balances in Recoupment Accounts) Requested by:" + this.owner);
			if (this.pdfRequired)
			{
	...
	public void invoke(DataSource ds, QueryDateBean qdb) throws QueryException
	{
		...
		buf.append("This message has been sent in response to your query request for Unpaid balances in Recoupment Accounts as the result of Emergency Checks issued. If you did not request this service or believe this message has been sent to you in error, please contact ACTS Help Desk..\n\n");
		...
		try
		{
			CallableStatement statement = connection.prepareCall("{call FKOS0001.FKAAS005(?,?,?,?,?,?,?,?,?,?,?,?,?)}");
			statement.setString(1, this.cdDisbType);
			statement.setString(2, this.idSubacct);
			...
			statement.registerOutParameter(10, 1);
			statement.registerOutParameter(11, 1);
			statement.registerOutParameter(12, 4);
			...
			try
			{
				String rf = this.path + System.getProperty("file.separator") + "QueryThreeDef.jasper";
	...
	{
		this.path = (QueryPropertyLoader.getPath() + System.getProperty("file.separator") + "QueryThree");
